...
<p>
This application (QueryClarity.cpp) computes clarity scores for an expanded 
query model based on pseudo-feedback documents. Performs the retrieval of 
those documents using the relevant SimpleKLRetMethod parameters.
Clarity scores for each entire query, and each individual term within each 
query are written to the file specified by the parameter "expandedQuery".
...
<font color=red> <em> different</em> interpretations</font>.  
<ul>
