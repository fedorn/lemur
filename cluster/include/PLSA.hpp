...
public:
  ...
  PLSA(const Index &dbIndex, int numCats, HashFreqVector **train, 
  ...
  PLSA(const Index &dbIndex, int testPercentage, int numCats, int numIter, 
       int numRestarts, double betastart, 
       double betastop, double anneal, double betaMod);
...
private:
  ...
  /// train d->w freq list
  HashFreqVector **data; // passed in
  /// test (validation) d->w freq list
  HashFreqVector **testData; // passed in
  ...
  double doLogLikelihood(jointfuncType, HashFreqVector **&myData);
