...
void indri::infnet::ExtentDescendantNode::prepare( int documentID ) {
  ...
  // check the inner extents, searching for an ancestor in outerNodes
  while ( innerIter != inExtents.end() ) {
	...
	while ( ancestor != _ancestors.end() && !found ) {
	  if ( docStruct->ancestor( *leaf, *ancestor ) ) {
	    found = true;
...
const indri::utility::greedy_vector<indri::index::Extent>& indri::infnet::ExtentDescendantNode::extents() {
  return _extents;
}
...
const std::string& indri::infnet::ExtentDescendantNode::getName() const {
  return _name;
}

void indri::infnet::ExtentDescendantNode::annotate( class Annotator& annotator, int documentID, int begin, int end ) {
...
void indri::infnet::ExtentDescendantNode::indexChanged( indri::index::Index& index ) {
  // do nothing
}
...
const indri::utility::greedy_vector<indri::index::Extent>& indri::infnet::ExtentDescendantNode::matches( indri::index::Extent extent ) {
  ...
  for( size_t i = 0; i < _extents.size(); i++ ) {
	...
	if ( *leaf != *ancestor && docStruct->ancestor(*leaf, *ancestor) ) {
	  match = true;
	}
	leaf++;
      } 
