...
/*====================================================================
 9/96 (ALB) Modified to support embedded ddparm's, to be resolved at
 run-time; e.g.:
   homedir=/u/clinton;
   bin=@(homedir)/bin;
   lib=@(homedir)/lib
   foo=@homedir; [without (), embedded ddparm assumed to extend to end]
====================================================================*/
...
void parse_stream(FILE *instream, char *inname){
   ...
   while (ddinf != NULL) {
	 ...
	 switch (c) {
	  case DELIMIT_CHAR: class = DELIMITER; break;
	  case ENDMARK_CHAR: class = ENDMARK; break;
	  case EOL:          class = END_OF_LINE; break;
	  case COMMAND_CHAR: class = COMMAND; break;
	  case QUOTE_CHAR:   class = QUOTE; break;
	  case DQUOTE_CHAR:  class = DQUOTE; break;
	  case FIXED_CHAR:   class = FIXIT; break;
	  case '/':  
	 ...
	 switch (state) {
	  ...
	  case ENDMARKED:
	    switch (class) {
	     ...
	     case END_OF_LINE: state = BEGIN_LINE; break;
             default: ddinf_error("Illegal character after end marker.");
	    }
	    break;
	  case COMMENT:
            switch (class) {
	  ...
	  case ARGUMENT:
	    ...
	    break;
	  case QUOTATION:
	    if (class == QUOTE) state=QUOTATION_ENDED;
	    else ddvalue_buffer[valueBufferIndex++] = c;
	    break;
	  case DQUOTATION:
	    if (class == DQUOTE) state=DQUOTATION_ENDED;
	    else ddvalue_buffer[valueBufferIndex++] = c;
	    break;
...
/*******************************************************/
void param_putc(const char *var_name, const char *val){
    param_init_restart();
