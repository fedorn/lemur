...
public:
  ...
  StopwordDictEntryFilter(const string &stopwords) {
    stopper = TextHandlerManager::createStopper(stopwords);
  }
  /// \brief Accept or reject an entry based on type.
  /// @param entry The entry to filter
  /// @return true if the entry's type is equal to the filter type value.
...
private:
  ...
  void writeTOC() const;  
  /// known term?
  bool contains(const string &term, Keyfile &keyfile) const;
  ...
  mutable Keyfile dict;
  /// target vocab termName -> freq table (delete if f == 0).
  mutable Keyfile targetIDs;
  /// File for entry data // mutable?
  mutable File dictEntries;
};
